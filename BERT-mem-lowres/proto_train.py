...
def log_all(y_pred, y_true, prefix, log_token, log_entity, step):
    ...
    if log_token:
        ...
        class_acc = (pred == y_true).float().mean()
        num_classes = num_labels - 3  
        class_accuracies = {}
        for i in range(1, num_classes):
            mask = y_true == i
            if mask.sum() > 0:  
                class_accuracies[f"token/{prefix}-class-{i}-accuracy"] = (pred[mask] == i).float().mean().item()
            else:
                class_accuracies[f"token/{prefix}-class-{i}-accuracy"] = 0.0  # 没有该类样本时，准确率设为 0
        print(f"Class accuracies for {prefix}:")
...
class TrueBert:
    def __init__(self, dataset):
        ...
        config = BertConfig.from_pretrained('bert-base-cased', num_labels=len(self.label_map), finetuning_task='ner',
                                                       output_hidden_states=True)
        print(config)
        # self.bert = BertModel.from_pretrained('bert-base-cased', from_tf=False, config=config).cuda()
        self.bert = BertModel.from_pretrained(config=config, from_tf=False, pretrained_model_name_or_path='bert-base-cased').cuda()
...
def train_proto(y, test_data, bert_model, epochs=1000, lr=1e-3, negative_sampling_ratio=4, l1=2, l2=3, support_min=1, support_other=100,
                embedding_dimension=300):
    """
    
    Args:
        y(torch.Tensor): Tensor of shape (n, m) where n is the number of sentences and m is the number of labels
    ...
    print("min support", len(idxs_sent_min))
    print("other support", len(idxs_sent_other))
    ...
    if args.lora:
        model = Lora(model, rank=10, alpha=4).cuda()
        bert_model.bert = Lora(bert_model.bert, rank=10, alpha=4).cuda()
    ...
    for epoch in trange(epochs):
        ...
        # print("l1 label nums:", (support_labels == l1).sum())
        # print("l2 label nums:", (support_labels == l2).sum())
...
def finetune_proto(y, test_data, bert_model, epochs=1000, lr=1e-3, negative_sampling_ratio=4, l1=2, l2=3, support_min=1, support_other=100,
                embedding_dimension=300):
    """
